#!/bin/bash
# Scorer (original SelfBalancingGradDiff) — Ablation: no retain term in main loss
# Uses SBGradDiffMatchedNoRetain class with the original ScorerTrainerGradDiff scorer trainer.
...
# ── HP search summary (uses sb_bayesian_summary.json — populated by HP search) ──
...
HP_KEY="${HP_KEY:-SBGradDiffNoRetain}"

# ── Hard-fail with a clear message if HP entry is missing ──
if ! python3 -c "
import json, sys
...
try:
    ...
    sys.exit(f\"HP file not found: {path}\")
if key not in d:
    sys.exit(f\"HP_KEY '{key}' not in {path}\")
bp = d[key].get('best_params', {})
missing = [k for k in ('lr', 'gamma', 'beta', 'scorer_lr') if k not in bp]
if missing:
    sys.exit(f\"{key} is missing required params: {missing}\")
" "$SB_SUMMARY_JSON" "$HP_KEY"; then
    echo "" >&2
    echo "Run the HP search first:" >&2
    echo "  python scripts/hpsearch_sb_methods.py --methods SBGradDiffNoRetain --n-trials 50 --gpus 0 --resume" >&2
    exit 1
fi
...
SCORER_scorer_lr=${SCORER_scorer_lr:-$(get_param "$HP_KEY" scorer_lr)}

task_name="SB_TOFU/${model}/forget10/ScorerAblations/no-retain"
...
echo ""
echo "============================================"
echo " Scorer Ablation: no-retain — Unlearn (forget10)"
echo "============================================"

HYDRA_FULL_ERROR=1 CUDA_VISIBLE_DEVICES=$GPU \
    python src/train.py --config-name=unlearn.yaml \
    experiment=unlearn/tofu/default \
    trainer=SBGradDiffNoRetainLearned \
    model=$model \
    task_name=$task_name \
    ...
    trainer.method_args.scorer_trainer.optim_cfg.lr=$SCORER_scorer_lr \
...
echo ""
echo "============================================"
echo " no-retain — Paraphrase Generation (forget10)"
echo "============================================"
...
echo ""
echo "============================================"
echo " no-retain — Paraphrase Judging (forget10)"
echo "============================================"
