...
SBNPO_lr=${SBNPO_lr:-$(get_param SBNPO lr)}
SBNPO_gamma=${SBNPO_gamma:-$(get_param SBNPO gamma)}
SBNPO_alpha=${SBNPO_alpha:-$(get_param SBNPO alpha)}
SBNPO_beta=${SBNPO_beta:-$(get_param SBNPO beta)}
SBNPO_scorer_lr=${SBNPO_scorer_lr:-$(get_param SBNPO scorer_lr)}
echo ${SBNPO_scorer_lr}

task_name="SB_TOFU/${model}/forget10/SBNPO"
...
HYDRA_FULL_ERROR=1 \
    ...
    trainer.args.learning_rate=$SBNPO_lr \
    trainer.method_args.gamma=$SBNPO_gamma \
    trainer.method_args.alpha=$SBNPO_alpha \
    trainer.method_args.beta=$SBNPO_beta \
    ...
    trainer.method_args.scorer_trainer.optim_cfg.lr=$SBNPO_scorer_lr \
