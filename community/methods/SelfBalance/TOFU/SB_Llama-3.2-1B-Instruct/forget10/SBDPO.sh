...
SBDPO_lr=${SBDPO_lr:-$(get_param SBDPO lr)}
SBDPO_gamma=${SBDPO_gamma:-$(get_param SBDPO gamma)}
SBDPO_alpha=${SBDPO_alpha:-$(get_param SBDPO alpha)}
SBDPO_beta=${SBDPO_beta:-$(get_param SBDPO beta)}
SBDPO_scorer_lr=${SBDPO_scorer_lr:-$(get_param SBDPO scorer_lr)}

task_name="SB_TOFU/${model}/forget10/SBDPO"
...
HYDRA_FULL_ERROR=1 \
    ...
    trainer.args.learning_rate=$SBDPO_lr \
    trainer.method_args.gamma=$SBDPO_gamma \
    trainer.method_args.alpha=$SBDPO_alpha \
    trainer.method_args.beta=$SBDPO_beta \
    ...
    trainer.method_args.scorer_trainer.optim_cfg.lr=$SBDPO_scorer_lr \
