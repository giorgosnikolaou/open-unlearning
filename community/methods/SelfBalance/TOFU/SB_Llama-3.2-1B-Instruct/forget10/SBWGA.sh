...
SBWGA_lr=${SBWGA_lr:-$(get_param SBWGA lr)}
SBWGA_gamma=${SBWGA_gamma:-$(get_param SBWGA gamma)}
SBWGA_alpha=${SBWGA_alpha:-$(get_param SBWGA alpha)}
SBWGA_beta=${SBWGA_beta:-$(get_param SBWGA beta)}
SBWGA_scorer_lr=${SBWGA_scorer_lr:-$(get_param SBWGA scorer_lr)}

task_name="SB_TOFU/${model}/forget10/SBWGA"
...
HYDRA_FULL_ERROR=1 \
    ...
    trainer.args.learning_rate=$SBWGA_lr \
    trainer.method_args.gamma=$SBWGA_gamma \
    trainer.method_args.alpha=$SBWGA_alpha \
    trainer.method_args.beta=$SBWGA_beta \
    ...
    trainer.method_args.scorer_trainer.optim_cfg.lr=$SBWGA_scorer_lr \
