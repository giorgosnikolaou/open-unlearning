#!/bin/bash
# RWKU eval (no fluency) for all Scorer (GradDiffScored) bayesian trials.
...
method="GradDiffScored"
# trial_base="/tmlscratch/nikolaou/RWKU/LLaMA-Factory/hyperparam/rwku_div10/${method}"
...
for trial_dir in "${trial_base}"/trial_*; do
    ...
    HYDRA_FULL_ERROR=1 CUDA_VISIBLE_DEVICES=0 \
        ...
        # paths.output_dir=$(pwd)/saves/unlearn/SB_RWKU/lr_sweep/${method}/${trial_name}
done
